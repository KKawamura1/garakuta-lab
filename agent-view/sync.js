...
const RULESET_VERSION = { arc: "arc-0.1", bus: "bus-0.3", phase: "phase-0.1", relay: "relay-0.1", laws: "laws-0.5", cost: "cost-0.1", ident: "ident-0.3", skip: "skip-0.4", squeeze: "squeeze-0.1", mutate: "mutate-0.2" };
...
export function buildPayload(session) {
  ...
  return {
    ...
    // 同定の版では、**当てにいったかどうかが観測したい量そのもの**なので、
    // アンケートと一緒に載せる（当てた法則と、言った回数）。
    answers: {
      ...(session.survey || {}),
