...
// **本命は 2026-08-23 に連勝機関へ移した**（作者が5点を付けた唯一の版）。
assert.match(app, /params\.get\("ruleset"\) \|\| defaultRuleset\(\)/, "新規セッションの既定は defaultRuleset");
assert.match(app, /return lawVariants\.length \? "skip" : "relay";/, "表が空なら遊べる版に落ちる");
...
// 飛ばしは**遊ぶ側が押したときだけ**起きること（SKIP 0.3、2026-08-23）。
//
// 元は報酬を取った直後に自動で走らせていた。初ランの記録で、第4戦と第5戦が
// そのまま自動で飛び、**構築画面が一度も出ていない**ことが分かった。
// 「強くなるか、続けるか」がぶつかる、と遊び方に書いておきながら、
// **ぶつかる場所が画面に無かった。**押す口を構築画面に置く。
assert.ok(!/type: "take"[\s\S]{0,600}?runSkips\(\)/.test(app), "報酬を取った直後に自動で飛ばしている");
assert.match(app, /function skipOfferCard/, "飛ばすかどうかを選ぶ札が無い");
assert.match(app, /if \(session\.touched\) return null;/, "触ったあとにも飛ばしを申し出ている");
assert.match(app, /skipOfferCard\(o, rules\)/, "飛ばしの札が構築画面に出ていない");
