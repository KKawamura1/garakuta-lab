...
function main(){const limit=Number(process.env.SEED_LIMIT||10000), checkpoint=process.env.CHECKPOINT||null, saved=checkpoint&&existsSync(checkpoint)&&statSync(checkpoint).size?JSON.parse(readFileSync(checkpoint,"utf8")):null,a=gateA(),f=gateF(),counts=saved?.counts??{B:0,C:0,D:0,E:0,BC:0,BD:0,CD:0,BCD:0,BCDE:0,all:0},first=saved?.first??{},top=saved?.top??[];for(let seed=saved?.nextSeed??1;seed<=limit;seed++){const optimal=campaign(seed,unrestricted);const b=gateB(optimal),c=gateC(seed,optimal),d=optimal.rows[0]?gateD(optimal.rows[0]):{pass:false},e=gateE(seed,optimal);const flags={B:b.pass,C:c.pass,D:d.pass,E:e.pass};for(const k of Object.keys(flags))if(flags[k]){counts[k]++;first[k]??=seed;}for(const [k,ok] of [["BC",flags.B&&flags.C],["BD",flags.B&&flags.D],["CD",flags.C&&flags.D],["BCD",flags.B&&flags.C&&flags.D],["BCDE",flags.B&&flags.C&&flags.D&&flags.E],["all",flags.B&&flags.C&&flags.D&&flags.E]])if(ok){counts[k]++;first[k]??=seed;}top.push({seed,score:Object.values(flags).filter(Boolean).length,flags,optimal,b,c,d,e});top.sort((x,y)=>y.score-x.score||x.seed-y.seed);top.length=Math.min(top.length,5);suffixCache.clear();battleCache.clear();if(checkpoint&&seed%100===0)writeFileSync(checkpoint,JSON.stringify({nextSeed:seed+1,counts,first,top}));}const result={ruleset:"control-0.2-calc-audit",limit,gateA:a,gateF:f,counts,first,top,overallPass:counts.all>0,uiDeploymentAllowed:false};if(checkpoint)writeFileSync(checkpoint,JSON.stringify({nextSeed:limit+1,counts,first,top,complete:true}));console.log(JSON.stringify(result,null,2));process.exitCode=0;}
main();
