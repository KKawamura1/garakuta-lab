...
try {
  ...
  // R18 — 取得は取り消せず、取得後は順番とオン／オフを調整できる。
  ...
  note("技能の手動装着操作が無い", await page.locator('[data-action="equip-skill"]').count() === 0);
  note("取得済み技能が自動で一覧に並ぶ", await page.locator(".skill-list-row").count() > 0);
  note("取得済み技能をオン／オフできる", await page.locator('[data-action="toggle-skill"]').count() > 0);
  note("行動と反応の順番を変えられる",
