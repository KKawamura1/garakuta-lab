...
作成日: 2026-08-31（UTC）  
更新日: 2026-08-31（UTC。作者指摘を受け、数値成長・攻撃パック配信・有限治療を追記）  
対象: EXP-18 Phase B 完了後の戦闘構築、難易度0〜20、Phase C着手前  
状態: 設計提案。コード変更・作者プレイによる支持はまだない。  
前提: [R4 創発的ルール生態系](./R4_EMERGENT_RULE_ECOLOGY_AND_LONG_TERM_EXPANSION.md)、[R6 長期進行](./R6_LONG_TERM_PROGRESSION_PROCEDURAL_LOOT_AND_BLUEPRINTS.md)、[R7 実装順序](./R7_IMPLEMENTATION_SEQUENCE_AND_PARALLEL_CONTENT_EXPANSION.md)、[R9 遠征HP持越し・有限治療・結果完全開示](./R9_EXPEDITION_ATTRITION_BOUNDED_HEALING_AND_EXACT_FORECAST.md)、[A9 Phase B実装結果](./A9_PHASE_B/RESULT.md)。
...
- R9に従い、通常・精鋭勝利後のHPをRunStateへcommitし、4戦目・8戦目boss後だけ全回復する。
- 自由回復を、被弾直後の応急処置、補給治療、遠征中に戻らない有限chargeへ分類する。
- 補給のcamp治療と、戦闘前の安全撤退を追加する。
- 次戦の勝敗、終了HP、消費資源、主要chainを副作用なしでexact previewする。
- HP持越しだけを先に作者公開しない。治療、撤退差、previewがそろってから4戦区間を評価する。
...
- 敵を残してroundを延長しても、遠征中に戻らない有限資源を使わずに次戦HPを改善できない。
- 通常・精鋭後のHP持越し、boss後回復、敗北非commit、retry snapshot、preview一致が機械検査される。
- 作者が4戦区間で治療・reroll・続行・安全撤退を比較し、12戦がHP節約だけの重いrunにならない。
...
| R6 §4.3「HPはBattleStateだけ・通常戦後全回復」 | 撤回。通常・精鋭勝利後はRunStateへHPをcommitし、boss後だけ拠点全回復する |
| R6 §5.2 baseline回復 | 自由回復ではなく、被弾直後の応急処置または有限治療へ置き換える |
| R6 §12.1 補給3用途 | camp治療を加え、retry・reroll・偵察・治療の4用途で競合させる |
| R6 §12.2 敗北と放棄 | 戦闘前の安全撤退と敗北のBlueprint保存数を区別し、撤退判断を成立させる |
| R4 / R8 余剰回復 | 実回復1以上かつ有限な支払いがある場合だけ発生させ、full HPへの空撃ちで生成しない |
