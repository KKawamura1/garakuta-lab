...
const lateShow = runBattle({ ...canonical, stage: MAX_STAGES - 1, activeCars: ["loop", "cut", "magnet", "reverse", "melt"], phase: "build" });
...
const armourCounter = runBattle({ ...canonical, stage: 2, activeCars: ["accelerator", "cut", "press"], phase: "build" });
assert.equal(armourCounter.report.won, true, "dense weight is an armour solution");
assert.ok(runBattle({ ...canonical, stage: 2, activeCars: ["charge", "melt"], phase: "build" }).report.won, "molten force is a second armour solution");
assert.equal(runBattle({ ...canonical, stage: 2, activeCars: ["charge"], phase: "build" }).report.won, false, "a light spark shot is visibly stopped by armour");

const fastCounter = runBattle({ ...canonical, stage: 4, activeCars: ["accelerator"], phase: "build" });
assert.equal(fastCounter.report.won, true, "a short accelerated line beats the fast cannon");
const fastLong = runBattle({ ...canonical, stage: 4, activeCars: ["accelerator", "press", "charge", "melt", "magnet"], phase: "build" });
assert.equal(fastLong.report.won, false, "a five-car line is a real time tradeoff against the fast cannon");
assert.ok(fastLong.report.events.some((event) => event.type === "impact_blocked" && /長い列/.test(event.note)));
assert.ok(fastLong.report.events.some((event) => event.type === "enemy_attack" && event.damage > 0));
assert.ok(runBattle({ ...canonical, stage: 4, activeCars: ["charge", "melt"], phase: "build" }).report.won, "a different short molten line also beats the fast cannon");

const scavengerCounter = runBattle({ ...canonical, stage: 3, activeCars: ["charge", "collector"], phase: "build" });
...
const scavengerLoss = runBattle({ ...canonical, stage: 3, activeCars: ["charge", "press"], phase: "build" });
...
assert.ok(runBattle({ ...canonical, stage: 3, activeCars: ["accelerator", "press"], phase: "build" }).report.won, "early destruction is a second scavenger solution");
...
assert.ok(bossReturn.report.events.some((event) => event.type === "impact_blocked" && /帰還弾/.test(event.note)));
