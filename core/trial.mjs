...
// **3戦から2戦へ。**作者：「2ラン連続でしかも結構重いゲームをやってると…なかなかつらい」。
// 1組の所要時間が3分の2になる。操作を強くしてあるので、2戦でも差は出るはず。
const BATTLES = 2;
...
export const TRIALS = {
  ...
  // 離した量：順序が効く 38%/1%
  // 【判定：支持。2026-08-22】**有効な4組のうち、位相なしを選んだ組はゼロ。**
  //   位相あり3／どちらとも1／位相なし0（引き分けを除けば3組で全会一致＝12.5%）
  // しかも**理由が操作した変数を名指ししている**（登録規則の「支持」の条件）：
  //   「2本目は位相なし。『ちょうど3つで強化』と相性が悪すぎる。」
  //   引っかかり「2戦目 試行錯誤の価値が低かった」← 位相を外すと試行錯誤に値段が付かない
  // **引き分けの扱いは登録に書いていなかった。**符号検定の慣習どおり分母から外した。
  // 迷った跡を残すために書いておく：4組中3組でも、逆側は一度も選ばれていない。
  // **休止。**決着した対に作者の時間を使わせない（0.9）。
  t3: {
    id: "t3",
    retired: true,
  ...
  // **1組目が「どちらとも言えない」だったので、操作を強くした。**
  //   前：勝てる並び 13% 対 59%（4.5倍）→ 気づかれず
  //   後：**11% 対 96%（9倍）。緩い側は「どう並べても勝つ」**（並びの当り100%）
  // これで気づかれないなら、T2 は面白さを動かしていない。**組を足すのではなく、操作を強くする。**
  // 実測：詰みなし 94%/100%
  t2: {
    id: "t2",
    ...
    sides: [
      ...
      { key: "loose", laws: ["relay", "vanguard"], phaseless: false, hpScale: 0.4 }
    ]
  },

  // **「閃き」の対。**噛み合わせが多いことは、面白さの条件か。
  //
  // 作者の感情マーカーで最も強かったのは、勝敗でも等級でもなく**気づき**だった：
  //   「そうか、一撃99の制限は2回攻撃なら198までいけるのか。知識アンロック！」
  // その中身は**機構どうしの噛み合わせ**である（敵の命中上限 × 部品の多段攻撃）。
  // 登録した条件（P1〜P12）はどれもこれを名指ししていない。**当たっている的かもしれない。**
  //
  // 操作する量：`analysis/interaction.mjs` の「噛み合わせ」
  //   = 法則2つを一緒に使った結果が、**単独の効果を足した予測からどれだけ外れるか**。
  //   外れない＝足し算で分かる＝閃きの余地が小さい。外れる＝組み合わせて初めて分かる。
  //
  // **順序が効くかも揃えてある。** t3 は3組とも位相ありが選ばれていて、
  // 順序が効くことは既に知覚される差だと分かっている。そこが揃っていないと、
  // 選ばれた理由が噛み合わせなのか順序なのか言えない。
  //
  // 実測（`analysis/match-insight.mjs`、91組から選んだ）：
  //   揃えた量：詰みなし 100%/100%、選択に勝目 79%/84%、並びの当り 63%/63%、順序が効く 77%/78%
  //   離した量：**噛み合わせ 57% 対 2%（28倍）**
  // 選択も並びも緩めなのは、揃えを優先した結果である（T2 の帯からは外れている）。
  // T2 自体が検証中の仮説で、しかも今のところ知覚されていないので、そちらを優先しない。
  //
  // **稼働。**t3 が決着したので入れ替える（2026-08-22）。
  // t2 は1組済みなので、`pickTrial` は組数の少ないこちらから出す。
  insight: {
    id: "insight",
    question: "機構が噛み合うことは、面白さの条件か",
    battles: BATTLES,
    sides: [
      { key: "interlocking", laws: ["overload", "fade"], phaseless: false, hpScale: 0.5 },
      { key: "additive", laws: ["counter", "monotony"], phaseless: false, hpScale: 0.7 }
    ]
  },
  ...
  // **休止。**1組目が空振りで、しかも空振りの理由が機構として説明されていた：
  //   「4T撃破を達成できたから」と言って**無傷基準の側**を選んでいる。
  //   巡回数への熱は等級が作っているのではなく、**予告が「N巡で撃破」と出し、
  //   自己最高が巡回数を持っている**——その2つで既に成立している。
  //   最上位等級の名札を掛け替えても、狙う対象は変わらなかった。
  // もう1組取るより、当たっている的（答え合わせ）に時間を回す。
  speed: {
    id: "speed",
    retired: true,
