...
const activeMeta = {
  ...
  relay_order: ["号令", "攻撃を一手休み、前列の先頭にいる自分以外の味方へ行動権を1渡す。", "指揮"],
  heavy_swing: ["溜め突き", "準備1回のあと、{amount}の一撃。開始と準備で行動権を計2つ使う。", "攻撃"],
  reposition: ["位置替え", "後衛なら、最も傷ついた前衛と場所を替え、自分に「守勢」を1つ。移動反応を起こしつつ前を引き受ける。", "機動"],
  ...
  steady_aim: ["狙いを澄ます", "攻撃を一手休み、自分に「集中」と反応権を1つずつ得る。集中中はスキップ。", "準備"],
  // R9 §5 — 横断pack「余波と受け渡し」の主行動。
  hand_off: ["引き継ぐ", "最も傷ついた味方へ受け構えを1つ渡してから、追い打ちを行う。", "支援"],
  overreach: ["無理を通す", "自分のHPが60%以上のときだけ、最もHPの低い敵へ{amount}。条件を欠いても行動は塞がらない。", "攻撃"],
  ...
  hamstring: ["足を払う", "腕力{amount}と引き換えに「怯み」を1つ付ける。怯んだ相手が出すダメージは1段につき15%減る（最大3段）。", "攻撃"],
  execute_low: ["首を落とす", "HP30%以下の敵がいるときだけ腕力{amount}。対象がいなければスキップ。", "攻撃"],
  rend: ["抉る", "腕力{amount}と「裂傷」2つ。裂傷はラウンド終わりに1段あたり最大HPの5%を、受けを無視して刻む。", "攻撃"],
  // 防壁と隊列（pack_wall）— 隊列の話を相手側へ広げる
  drag_forward: ["引きずり出す", "敵の後列で最もHPの低い一体を最前へ引き出し、「隙」を1つ付ける。届かなかった刃を次の攻撃へつなぐ。", "機動"],
  shield_wall: ["盾の列", "前列の味方**全員**へ技術{amount}のラウンド防壁。一人へ厚く張る「傷へ盾を」の面展開。", "守り"],
  rally_line: ["陣を組み直す", "最も傷ついた前衛と、最も無事な後衛（自分以外）を入れ替え、前へ出た味方に「守勢」を1つ。自分は動かない。", "機動"],
  bulwark_of_will: ["意地の壁", "**開幕2ラウンドだけ**、自分へ最大HPの{amount}の戦闘中防壁。技術ではなく体で張る。", "守り"],
  spread_the_guard: ["構えを配る", "前列にいる自分以外の味方全員へ受け構えを1つ。自分用の「衝撃に備える」と使い分ける。", "守り"],
  ...
  steady_breath: ["息を渡す", "隊列の最後にいる「集中」していない味方（自分以外）へ「集中」を1つ。", "支援"],
  ward_ally: ["守勢を渡す", "「守勢」が上限でない最も傷ついた味方へ1つ。受ける各hitを1段につき20%軽くする。", "支援"],
  ...
  hasten_ally: ["背を押す", "攻撃を一手休み、隊列の最後にいる自分以外の味方へ行動権を1渡す。号令と逆向き。", "指揮"],
  ...
  take_the_wound: ["傷を引き受ける", "最も傷ついた自分以外の味方へ「守勢」を2つ。代償として自分に「隙」が1つ付く。", "支援"],
  pass_the_edge: ["刃を渡す", "前列で「集中」していない最も速い味方（自分以外）へ「集中」を1つ。自分の一手を他人の大技へ渡す。", "指揮"],
};

const reactiveMeta = {
  counter_blow: ["反撃", "敵に殴られたあと、RP1で攻撃者へ腕力の{amount}ダメージ。", "被弾"],
  cover_ally: ["身代わり", "敵が自分以外の味方を狙った瞬間、RP1で自分へ引き受ける。自分が狙われた時は消費しない。", "標的"],
  ...
  // 回復量は被弾量ではなく所有者の技術40%。被弾は発動条件と回復窓だけを決め、
  // 同じ chain の隊全体で受けたHPダメージを超える分は engine が止める。
  mend: ["手当て", "誰かが被弾した直後、RP1で技術の{amount}を回復。同じ一撃を二重には治せない。", "回復"],
  triage: ["応急手当", "被弾後にHP半分以下になった自分以外の味方へ、RP1で技術の{amount}を回復。自分は対象にしない。", "回復"],
  emergency_treatment: ["応急処置", "自分が被弾した直後、RP1で技術の{amount}を回復。", "回復"],
  // R9 §5 — 横断pack。6つの発生源を、それぞれ別の役割の資源へ渡す。
  spill_forward: ["余波を回す", "過剰ダメージが出たあと、RP1で最もHPの低い敵へ腕力の{amount}。掃除役へ回る。", "撃破"],
  ...
  readied_relay: ["支度を渡す", "自分の準備が完了したあと、RP1で隊列の最後の味方へ反応権を2渡す。深い受け渡しほど収支が増える。", "準備"],
  ...
  absorb_shock: ["衝撃を殺す", "自分に飛んでくるダメージを、RP1で{amount}減らす。ラウンド1回。攻撃規模に追従する守り。", "被弾"],
  ...
  shared_pain: ["痛みを分ける", "味方が受けたダメージを、回復前に技術の{amount}ぶん軽減し、その4割を自分が受ける。RP1。", "防御"],
...
const passiveMeta = {
  ...
  held_breath: ["余りを溜める", "行動後にも行動権が残っていたら、「集中」を1つ得る。複数行動の次の一手を強くする。", "指揮"],
...
const REACTIVE_FOREST = [
  node("mend",  // 手当て
    ...
    // 「上限を超える回復」が要る。直接治療は固定量だが、Stage 0 では回復窓と隊全体の
    // 上限で手当てを完結させ、余剰を別の負傷者へ回す上級連携をまだ見せない。前提に
    // 置くと、自分を繋ぐ一本を取るために余剰治療まで買わされる。
