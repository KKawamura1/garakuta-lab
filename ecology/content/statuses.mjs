...
statuses.launcher_observed = {
  ...
  duration: "round",
  uniquePerSide: true,
  priorityForAllyAttackTargets: true,
  consumeOnAllyActiveAttackBaseTarget: true,
  rules: [],
  tags: ["playable", "debuff", "launcher"],
};

statuses.launcher_exposed = {
  id: "launcher_exposed",
  displayName: STATUS_NAMES.launcher_exposed,
  polarity: "negative",
  maxStacks: "unbounded",
  duration: "battle",
  decayAtRoundEnd: true,
  rules: [{
    id: "launcher_exposed_damage_rule",
    listenTo: "damage_proposed",
    timing: "interrupt",
    priority: 46,
    predicates: [SELF_IS_EVENT_TARGET, { type: "event_tag", tag: "attack" }],
    costs: [],
    effects: [
      {
        type: "modify_pending_amount",
        operation: "increase",
        amount: {
          type: "pending_amount_times_status_scaled",
          subject: "self",
          statusId: "launcher_exposed",
          numerator: 1,
          denominator: 10,
        },
      },
...
const STATUS_SUMMARIES = {
  ...
  launcher_observed: "射出器が味方全員へ示す敵。次の味方の攻撃主軸で優先され、解決後に1段消費。最大6段で、別の敵を観測すると移る。",
  launcher_exposed: "敵が受ける攻撃ダメージが1段につき10%増える。付与者を問わず合算し、段数上限なし。ラウンド終了時に切り捨て半減。",
