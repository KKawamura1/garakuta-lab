...
// （R9 §7 の最後）。
//
// Campaignの物語イベントは初訪・再訪で分岐しない。同じ Stage を何度遊んでも、
// opening / join・幕の断片・stageEnd は同じ本文・演出で再生する。
...
export const PROLOGUE = Object.freeze({
  ...
  // R11 §5 改（作者指摘 2026-09-12）— 巻き戻したあとの並べ替えは**手取りの型**にする。
  // 押す場所が光り、その一手を打ち終えるまで他は押せない（DESIGN.md §6.4.4）。
  // **どの一手を教えるかは content が決める。**画面は光らせる先をここから引くので、
  // 人物 id と行を app.js へ書き写さない（教える一手を変えたら、錠と光も一緒に動く）。
  tutorial: Object.freeze({ characterId: "mender", row: "rear" }),
});

// ---------------------------------------------------------------- 技能の取得と予約（作者要望 2026-09-14）
//
// **シナリオ1の一戦目の後に、技能点の使い方を教える。**
//
// 作者要望 —「一戦目前: 隊列のチュートリアル / 一戦目後: スキル取得・予約の
// チュートリアル / 二戦目後: 補給のチュートリアル」。補給を一つ後ろへ送り、
