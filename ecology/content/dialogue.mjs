...
//
// **会話本文の正本。**本編の物語、序盤の灰の門、根城の日常場面で
// 表示する台詞と地の文を、ここへ一箇所に集約する。
//
// 会話の文言を変えるときは、このファイルだけを編集する。story.mjs と
// homestead.mjs には舞台・配役・条件・表示順だけを置き、会話担当と
// システム担当が同じ行を触らないようにする。
//
// タイトル、場所、立ち位置、解禁条件、説明用 footer / hint、名簿の設定文は
// 会話本文ではないため、それぞれの定義ファイルに残す。

import { narrate, say } from "./beat.mjs";
...
export function dialogueFor(beatId) {
  const lines = DIALOGUE[beatId];
  if (!lines) throw new Error("dialogue: 未登録の断片 " + beatId);
  return lines;
}
