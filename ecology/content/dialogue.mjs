...
export const DIALOGUE = Object.freeze({
  ...
  //
  //   ゴウ   … 声がでかい。減らず口。都合が悪いと逸らし、押し切られると「へいへい」
  //   ツグミ … 早口。被せる。丁寧語のまま容赦がない。数えて突きつける
  //
  // **ゴウを素直に折らない（R15・作者指摘）。**初稿は開幕で「……はい」と折っていたが、
  // 直後に一分で立って先に行くので**言行が一致していない**うえ、読み手には
  // 大男が急にしおらしくなる理由が無い。「へいへい」は口だけ合わせて従わない返事で、
  // ここでやることと一致する。**受けるのはツグミの「もう！」**で、丁寧語が一度だけ
  // 剥がれる。二人が対等だと見えるのは、折れる側がいないからである。
  //
  ...
  stage_0_opening: Object.freeze({
    ...
    lines: Object.freeze([
      ...
      say("warden", "へいへい。座る、座ります。", "wry"),
      narrate("坑の口から、ぬるい風が上がってくる。ゴウは座った。——一分で立って、先に行った。"),
      say("mender", "もう！ 三分って言いましたよね！ 三分！", "firm"),
    ]),
  }),
