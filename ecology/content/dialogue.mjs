...
export const DIALOGUE = Object.freeze({
  ...
  //
  // ---- 戦闘の結果に驚かせない（R16追補・作者指摘）-----------------------------
  //
  // 初稿は「……誰も落ちていません」と shock で始めていた。**先見機は結果まで読める**
  // （`PROLOGUE.retryHint`「次の一戦の結果が先に読める」、`app.js`「その戦闘結果は
  // 常に見えます」）。巻き戻したあとは予測が出るので、**校正した本人であるツグミは、
  // 数も並びも終わり方も知ったうえで入っている。**驚く理由が無い。
  //
  // 前稿の「さっきと同じ」（＝巻き戻り前の記憶）を潰したとき、驚きの理由だけが
  // 差し替わらずに残っていた。**戦闘後の彼女にとって新しい情報は、ゴウの様子だけである。**
  // 一行目は当てた側の確認にして、視線をそちらへ移す。
  stage_0_prologue_win: Object.freeze({
    cast: Object.freeze([stand("mender", "left"), stand("warden", "right")]),
    lines: Object.freeze([
      say("mender", "先見機の読みどおりでした。数も、並びも、終わり方も。……当たり前です。校正したのはわたしですので。", "calm"),
      say("warden", "だろ？ さっきは前に二人で並んじまったからな。今度は立つ場所を変えた。それだけだ。", "smile"),
      say("mender", "「さっき」。", "neutral"),
      say("warden", "言ってねえ。", "wry"),
      say("mender", "言いました。いま一度。門の手前でも一度。……三分待ったのも、先に中を見せろと言ったのも、今日が初めてです。", "firm"),
