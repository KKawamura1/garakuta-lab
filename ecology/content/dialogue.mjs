...
export const DIALOGUE = Object.freeze({
  ...
  stage_0_prologue_rewound: Object.freeze({
    ...
    lines: Object.freeze([
      ...
      say("mender", "見せますよ。あと三分待てば、次の一戦の中身が全部読めますから。数も、並びも、終わり方も。", "firm"),
  ...
  //
  // **本文は作者稿のままにする。**表記も直さない（「通り」「私」「『杭』」「手帳」）。
  // 一度こちらで「とおり」「わたし」「「杭」」「台帳」へ寄せ、さらに「もっと」を落として
  // 読点を足したが、**作者が書いた台詞を、断りなく整形してはいけない**（作者指摘）。
  // ツグミの一人称がここだけ「私」なのは、そのための例外である（他は「わたし」）。
  stage_0_prologue_win: Object.freeze({
    cast: Object.freeze([stand("mender", "left"), stand("warden", "right")]),
    lines: Object.freeze([
      say("mender", "目標沈黙。……ふう。先見機で見た通りでした。ほら、ちゃんと校正しておいてよかったでしょ、ゴウ？", "smile"),
      say("warden", "……そうだな。お前の言う通りだ。ありがとな。", "calm"),
      say("mender", "？ どうしました？ 何か変ですよ。いつもならもっと憎まれ口の一つでも叩くところなのに。", "worry"),
      say("warden", "うるせえ。さっさと先に進むぞ。", "wry"),
      say("mender", "……もしかして。『杭』、使いました？ 私、死にました？", "shock"),
      say("warden", "……あーそうだよ。俺が悪かったんだ。お前は死んだ。『杭』で戻った。それだけだ。", "hurt"),
      say("warden", "早く行こうぜ。手帳にも書かない。さっさと帰って、全部忘れよう。", "worry"),
    ]),
  }),
  ...
  stage_1_act1: Object.freeze({
    ...
    lines: Object.freeze([
      ...
      say("mender", "顔が赤いです。脈も上がっています。……褒められただけで、怪我をした人と同じ数字になってますよ。大丈夫ですか、それ。", "neutral"),
