...
//
// baseline の `strike`・`bulwark`・`mend` も初期習得技能として扱う。初期習得技能は
// `freshLoadout` が種別に応じた一覧へ自動で反映する。`strike` が使えないときの
// 通常攻撃 fallback（skills-active.mjs の coreStrike）は別の安全弁であり、取得状態を表さない。
//
