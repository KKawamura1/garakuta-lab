...
| `analysis/ecology-stage0-builds.mjs` | Stage 0 の三構成（issue #176）を data として持ち、取得計画の予算・核の成立時点・代表装備・同じ seed での event 列の違いを、実際に engine へ通して検査する smoke。 |
...
`sortValue` 一箇所です。`hp_asc` / `hp_desc` は残りHPそのもの、`hp_percent_asc` /
`hp_percent_desc` は**傷の割合**で、値は `hp * 10000 / maxHp` の切り捨て（整数 bps）から作ります。
