...
| `weapon-skill-prototype.html` / `weapon-skill-prototype.js` / `weapon-skill-prototype.mjs` / `weapon-skill-prototype.css` | Stage 4 の独立UIプレビュー。190節のカタログをPR #289の地図／一覧・役割丸記号・効果バッジ・効果のみの詳細盤で表示する。内部位置名を画面へ出さず、技能の取得・保存・戦闘には接続しない |
