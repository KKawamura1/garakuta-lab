...
| `weapon-skill-prototype.html` / `weapon-skill-prototype.js` / `weapon-skill-prototype.mjs` / `weapon-skill-prototype.css` | Stage 4 の独立UIプレビュー。190節をPR #289の地図／一覧・役割丸記号・効果バッジ・効果のみの詳細盤で表示し、固定loadout fixtureと予約デモも示す。内部位置名を画面へ出さず、本編の取得・保存・報酬・戦闘には接続しない |
