...
見取りは隣接味方が得た強化と同じ段数、重ね構えは1段を追加する。無手は連携も含めた強化を消費せず、種類ごと最大6段まで数える。
...
- 号令はAP移譲や強化だけを行ってよい。号旗BAは借りと高揚を明示し、借りの未返済段数を持ち越す。号旗BBは守勢の段数で全体防壁と集中を強める。
- 鉤縄BB3は敵1体を最大2マス引き、隙2を与える。移動できなくても攻撃は成立する。
- 重弩BAは準備中の弱射撃と長い装填、重弩BBは双刃とも共有する仕込みを次弾装填・速射準備へ使い、極限装填は長い準備で威力を得る。未来の行動を予約して消す処理は持たない。
