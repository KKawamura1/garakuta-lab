...
高揚・守勢・堅牢・破甲・裂傷・再生・露呈・移動不能の8種類は段数上限を設けず、終了時に合計段数を切り捨て半減する。他の状態は上限6段、連携だけは300段とする。個々の意味、消費、持続はカタログ0.3節を正本とする。観測・露呈は敵に付く弱体、準備は中立の状態である。
...
見取り・重ね構えは強化を1段、再生だけは2段追加でき、無手は連携も含め最大6段を消費する。
複製による付与を同じ複製技能へ再入力しない。再生を複製しても2段を追加するだけで、付与者や技術値は保持しない。

持続状態は段ごとの時間を持たず、ラウンド終了時に合計段数を`floor(段数/2)`へ更新する。1段なら消える。終了時の付与は半減後に行い、次の終了時から半減する。
盾役は複数回攻撃を受けるため、守勢1の被ダメージ軽減は4%（合計最大80%）、堅牢1の受け上昇は各hitで+2とし、
集中1の次の主軸+30%より低い割合を選ぶ。怯みは1段-15%で、6段でも主軸を完全停止しない。再生は終了時に残った1段につき次の開始時に固定の防壁5。再生は医療具からの直接付与と見取り・重ね構えの複製を含め、最低2段ずつ付ける。数値は戦闘標本で検証する。
