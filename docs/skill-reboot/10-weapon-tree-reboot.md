...
- 個別効果も実装へ同期する。医療具のactiveは直接HP回復から防壁中心へ変更し、再生薬・予備血は現行player registryでは防壁の再生成として扱う。回復効果そのものは大盾の被弾リアクティブなどに残し、蘇生は汎用`revive`と`actor_revived`で扱う。
...
「主軸を強くする節」だけでは、どの武器もR→A→AAを伸ばす一本道になる。そこで各武器に、医療具の「応急手当」と同じく、**主軸とは別の出来事を読んで武器の性格に近い価値へ変える副接続節**を最低一つ置く。
...
- 同じ`damage_taken`でも出口を武器ごとに変える。戦槌は怯み付き反撃、射出器は援護照準、医療具は瀕死者の回復、大盾は被害分散、双刃は裂傷付き反撃とする。

今回のカタログでは、19節×10武器の枠を増やさず、数値強化に寄っていた10節をこの副接続へ置き換える。分類はアクティブ70／リアクティブ35／ターゲット9／パッシブ76の合計190節となる。リアクティブが同じ反応窓で競合する場合は、従来どおり人物ごとの優先列で一つだけを解決するため、反応を増やしても同じ被弾へ無制限に重ならない。
...
- 被弾後の回復や蘇生はリアクティブとして許可する。医療具の再生薬・予備血は、現行実装では回復ではなく防壁の再生成へつなぐ。
- `号令`はAP移譲や強化だけを行ってよい。
