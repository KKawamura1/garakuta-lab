...
同じ単体攻撃へ副対象を追加する振り幅・貫通・炸裂筒は、主対象と対象変更を確定した後の同じ副対象拡張eventを共有する。条件を満たす複数を持っていても、優先列の最初の一つだけが発動する。別のevent、たとえば`target_selected`と`damage_resolved`にはそれぞれ一度反応できる。
...
6. 予定した対象人数、開始時の状態・資源、攻撃位置に基づいて効果量を固定する。対象数や倍率を命中後の結果で遡及変更しない。
7. APと費用を払い、各hitを命中→受け構え→受け→防壁→HPの順に処理する。命中・被弾eventの反応は未処理のhit以降にだけ効く。
