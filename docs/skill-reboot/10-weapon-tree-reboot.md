...
全種類の上限を6段で共通化し、連携だけは大量蓄積を残して300段とする。個々の意味、消費、持続はカタログ0.3節を正本とする。観測・露呈は敵に付く弱体、準備は中立の状態である。
...
防壁の破壊、HP・AP・RPの増減は段数の解除ではない。期限切れと使用による消費も解除に数えない。
見取り・重ね構えは強化なら種類を問わず1段追加でき、無手は連携も含め最大6段を消費する。
複製による付与を同じ複製技能へ再入力しない。再生を複製しても同じ1段を追加するだけで、付与者や技術値は保持しない。

長時間効果は段ごとに3ラウンド続き、再付与で古い段の期限を更新しない。上限6段まで重ねられる。
盾役は複数回攻撃を受けるため、守勢1の被ダメージ軽減は2%、堅牢1の受け上昇は各hitで+1とし、
集中1の次の主軸+30%より低い割合を選ぶ。怯みは1段-15%で、6段でも主軸を完全停止しない。再生は1段につき毎ラウンド固定の防壁5。数値は戦闘標本で検証する。
...
観測は味方全員で共有し、攻撃可能ならその敵一体を各人の攻撃主軸のターゲット規則より先に選ぶ。攻撃主軸一回の解決後に1段減り、多段・範囲でも一度だけ減る。追撃・反撃では減らない。別の敵へ付けると旧観測を消し、観測対象が攻撃不能なら段を消費せず各人の規則へ進む。露呈は敵ごとに付与者を問わず最大6段まで合算し、受けるダメージを1段につき+10%する。各段は3ラウンド続き、別の敵へ付けても残る。ターゲットは上書きしない。観測孔の追撃は観測の付与者ではなく、その技能を持つ本人が観測中の敵への味方の命中に反応して行う。
