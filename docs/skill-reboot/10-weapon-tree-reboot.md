...
3. 単体攻撃・単体支援・列／縦列への範囲攻撃ならターゲット優先列を解決し、基準となる対象を一体へ絞る。範囲はこの対象の位置から確定する。
...
- 一括反転: 鉤縄BB3のように味方・敵の前後列を一度だけ反転する効果は、固定位置順で解決し、解決後に自動で戻さない。反転で発生したactor_movedは各actor一度だけで、同じ反転を再び発生させない。
...
- 長槍や列攻撃の射線へ敵を並べる。
- 前進・後退・強制移動を、勢い、防壁、隙、反撃など別の技能の発生源にする。
...
横薙ぎの「2体以上」は、hit数ではなく、実際にダメージ判定を行った異なる敵の人数で数える。貫通の副対象と、命中後にRPを支払って発動した振り幅の左右の敵を含む。振り幅の第1hitが不命中、RP不足、反応優先列で不発、隣接敵不在なら数えない。人数が確定してから、主対象の第1hitと副対象の追加ダメージを含む攻撃全体へ+15%を一度だけ適用する。
