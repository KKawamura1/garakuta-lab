...
- 個別効果も実装へ同期する。医療具の標準activeは防壁中心だが、A→AA／A→ABは被弾リアクティブ回復を主役にし、B→BAは全体防壁、B→BBは残存防壁のラウンド末回復と戦闘終了時回復へ分ける。回復可能防壁のような新しい防壁種別は導入しない。再生薬は既存の状態付与、蘇生は汎用`revive`と`actor_revived`で扱う。
...
守勢は共通の被ダメージ軽減状態とし、1段-10%、最大3段、次ラウンド開始で消える。大盾の枝ごとに別の軽減カウンターを増やさない。
