...
範囲の攻撃は基準の敵一体をターゲット規則で決めてから形を広げる。「敵一列」はその敵のいる横3マス、「縦列」は同じ横位置の前後2マス。対象が倒れても、解決中の範囲や複数hitは明記がない限り再選択しない。単体攻撃に貫通などで副対象を足す時も、主対象を先に決める。反撃の攻撃者・追撃の同じ敵・誘引による変更はターゲット優先列を読まない。
...
攻撃の射程はその攻撃に定義された近接・長射程・遠隔で決め、元の武器名や腕力・技術の種類から推測しない。元の攻撃の基礎対象が一体なら、2hit以上でも「単体攻撃」である。副対象を追加しても、振り幅・貫通などの「単体攻撃」を条件にする技能の資格は失わない。

横薙ぎの「2体以上」は、hit数や実際の命中人数ではなく、攻撃前にRPを支払って確定した異なる予定対象の人数で数える。貫通や振り幅の副対象も含め、同じ敵は一人と数える。RP不足・有効な隣接敵不在なら振り幅は対象を追加しない。hitが外れたり途中で敵が倒れたりしても、主対象の第1hitと副対象への+15%を遡及して変更しない。
