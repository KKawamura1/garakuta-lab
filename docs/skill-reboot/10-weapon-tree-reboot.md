...
付与した有利な状態を強化、不利な状態を弱体と呼び、付与元ごとに保持する。受け+10と受け-20が同時にあっても、受け上昇を解除すれば+10だけが消える。複数の受け上昇を「すべて解除」するなら正の付与元をすべて消す。相殺後の値や最新一件を対象にしない。強化1つの除去を数える時は、段数や数値の大きさではなく、除去した状態の付与元1件を1つとする。

「段数を持つ正の状態」の複製・消費は集中・守勢などの状態カウンター1段を単位とする。受け+10を10段とは数えない。再生Nは残り期間なので段数に含めない。防壁・受け構え・誘引・HP・AP/RP・連携・破片・予約刃・次弾・借り・旗印は資源として別扱いにする。観測・集中標は敵への標識であり、味方の正の状態として写せない。防壁と受け構えは専用の除去効果だけで消す。弱体解除は怯み・裂傷などと不利な数値修正を付与元ごとに消す。

守勢は共通の被ダメージ軽減状態とし、1段-10%、最大3段、次ラウンド開始で消える。大盾の枝ごとに別の軽減カウンターを増やさない。受け無視は攻撃開始時に対象ごと合算し、その攻撃の各hitで同じ量だけ対象の受けを減らす。
...
観測は味方全員に共有する標識で、各人のターゲット規則がどれも成立しない場合だけ、最短距離の既定より先に選ぶ。複数の観測敵なら残り期間、距離、固定マス順で選ぶ。選べない敵の観測は飛ばす。集中標はその人物の与ダメージ条件であり、自動でターゲットを上書きしない。
...
- 号令はAP移譲や強化だけを行ってよい。号旗BAは借りとHPを明示し、号旗BBは時間延長ではなく旗印3の特殊勝利にする。
- 鉤縄BB3は敵味方の前後列を一度だけ反転する。元の隊列へ戻さず、移動した人数ぶんの反応だけを起こす。
- 重弩BAは準備中の弱射撃と長い装填、重弩BBは次弾を使う極限装填を行う。未来の行動を予約して消す処理は持たない。
