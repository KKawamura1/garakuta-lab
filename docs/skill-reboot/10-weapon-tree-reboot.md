...
見取り・重ね構えは強化なら種類を問わず1段追加でき、無手は連携も含め最大6段を消費する。
...
集中1の次の主軸+30%より低い割合を選ぶ。怯みは1段-15%で、6段でも主軸を完全停止しない。再生は終了時に残った1段につき次の開始時に固定の防壁5。医療具から再生を直接付ける場合は最低2段。見取り・重ね構えは1段ずつ複製する。数値は戦闘標本で検証する。
