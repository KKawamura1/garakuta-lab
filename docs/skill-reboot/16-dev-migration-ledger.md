...
- [x] **3c / PR #308 — skill packとequipment packの分離。** `ecology/weapon-pack-manifest.mjs` は武器10種のskill pack（`skill:<weaponId>`）と装備affix family pack（`equipment:<familyId>`）を別registry・Profile欄・Manifest欄として定義する。二つのpoolを別seed domainで抽選し、取得可能nodeはskill packから、装備familyはequipment packからだけ導出する。共通の `family_scar` は常時有効。旧Profile/Runと現行runtimeへはまだ接続しない。
