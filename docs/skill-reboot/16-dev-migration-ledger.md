...
更新日: 2026-09-25  
状態: **Stage 0・Stage 1完了。Stage 2の反応優先列・ActionPlan基礎・対象前移動・攻撃開始反応まで実装。** PR #291で190節のカタログ同期・ID出典整理・初期20節の導出を追加した。PR #292で敵の実使用技能だけを独立したregistryへ移し、schema・engine・敵戦闘の境界を切り替えた。PR #294で同一人物のリアクティブ優先列を監査仕様へ合わせ、PR #295で同一効果列に含まれる複数の直接ダメージを一つのActionPlanへ束ねた。PR #296で対象前の移動反応を解決順へ接続し、移動後に対象を再選択した。PR #297では確定した対象・APの後、主効果とActionPlan固定の前に攻撃開始時のinterruptを解決する。副対象拡張・追加hit/RP、防御崩し・hit後反応・攻撃後処理、武器別player registry、未実装技能の取得制御、新runtimeへの全面移行は後続作業。
...
- [x] PR #297で `action_started` のinterruptを攻撃開始時の反応窓にする。最終対象とAP支払いを固定してから一人物ごとのリアクティブ優先列を解決し、反応とそのafter反応後に主効果のActionPlanを作る。攻撃者を倒す反撃ではAPを返さず本体効果を中止し、対象を選び直さない。`ecology/engine.test.mjs` で開始時の強化が最初のdamageへ反映されること、先行候補がRP不足なら次候補が発動すること、攻撃者撃破時に主damageが発生しないことをevent traceで固定する。`action_started` ではpending-action専用の対象変更・取消し効果を拒否する。
